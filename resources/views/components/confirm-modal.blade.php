...
{{-- Modal Container --}}
<div id="{{ $id }}" style="display: none; position: fixed; inset: 0; z-index: 9999;" role="dialog"
    aria-modal="true">
    {{-- Backdrop --}}
    <div id="{{ $id }}-backdrop"
        style="position: fixed; inset: 0; background: rgba(0,0,0,0.5); opacity: 0; transition: opacity 0.2s;"></div>

    {{-- Modal Content --}}
    <div
        style="position: relative; height: 100%; display: flex; align-items: center; justify-content: center; padding: 1rem;">
        <div id="{{ $id }}-panel"
            style="background: white; border-radius: 1rem; padding: 1.5rem; width: 100%; max-width: 400px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); transform: scale(0.95); opacity: 0; transition: all 0.2s;">
            ...
            {{-- Title --}}
            <h3
                style="text-align: center; font-size: 1.125rem; font-weight: 600; color: #111827; margin-bottom: 0.5rem;">
                {{ $title }}
            </h3>

            {{-- Message --}}
            <p id="{{ $id }}-message"
                style="text-align: center; font-size: 0.875rem; color: #6b7280; margin-bottom: 1.5rem;">
                {{ $message }}
            </p>
            ...
            <div style="display: flex; gap: 0.75rem; justify-content: center;">
                <button type="button" id="{{ $id }}-cancel"
                    style="padding: 0.625rem 1.25rem; font-size: 0.875rem; font-weight: 500; color: #374151; background: #f3f4f6; border-radius: 0.5rem; border: none; cursor: pointer; transition: background 0.15s;"
                    onmouseover="this.style.background='#e5e7eb'" onmouseout="this.style.background='#f3f4f6'">
                    {{ $cancelText }}
                </button>

                <button type="button" id="{{ $id }}-confirm" class="{{ $buttonColor }}"
                    style="padding: 0.625rem 1.25rem; font-size: 0.875rem; font-weight: 500; color: white; border-radius: 0.5rem; border: none; cursor: pointer; transition: filter 0.15s;"
        ...
        (function() {
            ...
            confirmBtn.addEventListener('click', function() {
                if (onConfirmCallback) {
                    ...
                    var loadingText = confirmBtn.getAttribute('data-loading-text') || 'Memproses...';
            ...
            backdrop.addEventListener('click', function() {
                if (!confirmBtn.disabled) hide();
            });

            // Escape key
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape' && modal.style.display === 'block' && !confirmBtn.disabled) {
                    hide();
                }
            });
        })();
