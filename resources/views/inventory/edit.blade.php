    ...
    <section class="rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-black/60">Perbarui informasi produk.</p>
            </div>
        ...
        <form class="mt-6 grid gap-4 md:grid-cols-2" method="post" action="{{ route('inventory.update', $product->id) }}">
            @csrf
            @method('PUT')
            <div>
                <label class="mb-1 block text-xs font-semibold text-black/60">Nama Produk</label>
                <input name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="text" required />
                @error('name')
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    <input id="inventory-barcode-input" name="barcode" value="{{ old('barcode', $product->barcode) }}" class="flex-1 rounded-lg border border-black/20 bg-white px-3 py-2" type="text" required />
                ...
                <div class="mt-3 rounded-lg border border-dashed border-black/30 bg-white/70 p-4">
                    ...
                    <div id="inventory-scanner-preview" class="mt-2 flex h-40 items-center justify-center rounded-md border border-black/10 bg-black/5 text-xs text-black/50">
            ...
            <div>
                ...
                <select name="unit" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" required>
            ...
            <div>
                ...
                <select name="supplier_id" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2">
            ...
            <div>
                ...
                <input name="price_buy" value="{{ old('price_buy', $product->price_buy) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('price_buy')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="price_sell" value="{{ old('price_sell', $product->price_sell) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('price_sell')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                <label class="mb-1 block text-xs font-semibold text-black/60">Stok</label>
                <input name="stock" value="{{ old('stock', $product->stock) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('stock')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="min_stock" value="{{ old('min_stock', $product->min_stock) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('min_stock')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
