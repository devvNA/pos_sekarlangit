...
@section('content')
    <section class="rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
        ...
        <form class="mt-6 grid gap-4 md:grid-cols-2" method="post" action="{{ route('inventory.store') }}">
            @csrf
            <div>
                <label class="mb-1 block text-xs font-semibold text-black/60">Nama Produk</label>
                <input name="name" value="{{ old('name') }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="text" required />
                @error('name')
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    <input id="inventory-barcode-input" name="barcode" value="{{ old('barcode') }}" class="flex-1 rounded-lg border border-black/20 bg-white px-3 py-2" type="text" required />
            ...
            <div>
                ...
                <input name="price_buy" value="{{ old('price_buy') }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('price_buy')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="price_sell" value="{{ old('price_sell') }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('price_sell')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="stock" value="{{ old('stock', 0) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('stock')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="min_stock" value="{{ old('min_stock', 0) }}" class="w-full rounded-lg border border-black/20 bg-white px-3 py-2" type="number" min="0" required />
                @error('min_stock')
                    <p class="mt-1 text-xs font-semibold text-red-600">{{ $message }}</p>
                @enderror
            </div>
